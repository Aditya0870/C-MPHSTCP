...
struct mpbbr {
	...
	// RTT filter
	Time m_prevRtt;
	double m_rttAccumulator;
	uint32_t m_rttSampleCount;
	Time m_lastRttUpdate;
	Time m_rttFilterWindow;

	// ECN filter
	uint32_t m_ecnMarkedAck;
	uint32_t m_totalAck;
	Time m_lastEcnUpdate;
	Time m_ecnFilterWindow;

	// PL filter
	Time m_lastPacketLoss;

	// Shared Bottleneck Logic
	bool m_rttFilterPass;
	bool m_ecnFilterPass;
	bool m_isSharedBottleneck;
	uint8_t m_sbMatchCounter;
...
//Initialisations that we made
m_prevRtt = Seconds(0);
m_rttAccumulator = 0.0;
m_rttSampleCount = 0;
m_lastRttUpdate = Seconds(0);
m_rttFilterWindow = Seconds(1);

m_ecnMarkedAck = 0;
m_totalAck = 0;
m_lastEcnUpdate = Seconds(0);
m_ecnFilterWindow = MilliSeconds(250);

m_lastPacketLoss = Seconds(0);

m_rttFilterPass = false;
m_ecnFilterPass = false;
m_isSharedBottleneck = false;
m_sbMatchCounter = 0;
...
//Update we have made
static void UpdateSharedBottleneckDetection(Ptr<TcpSocketState> tcb, Time rtt, bool isEcnMarked)
{
    ...
    if (!m_prevRtt.IsZero())
    {
        double delta = std::abs((rtt - m_prevRtt).GetMilliSeconds());
        m_rttAccumulator += delta;
        m_rttSampleCount++;
    }
    m_prevRtt = rtt;

    if (now - m_lastRttUpdate >= m_rttFilterWindow)
    {
        double avgDelta = m_rttSampleCount > 0 ? m_rttAccumulator / m_rttSampleCount : 0;
        m_rttFilterPass = CheckRttSimilarityWithOtherSubflows(avgDelta);
        m_rttAccumulator = 0;
        m_rttSampleCount = 0;
        m_lastRttUpdate = now;
    }
    ...
    if (now - m_lastEcnUpdate >= m_ecnFilterWindow)
    {
        double ecnRate = m_totalAck > 0 ? (double)m_ecnMarkedAck / m_totalAck : 0;
        m_ecnFilterPass = CheckEcnSimilarityWithOtherSubflows(ecnRate);
        m_ecnMarkedAck = 0;
        m_totalAck = 0;
        m_lastEcnUpdate = now;
    }
    ...
    // --- Packet Loss Filter ---
    bool plFilterPass = CheckRecentLossSimilarity(m_lastPacketLoss);

    // --- Final Decision ---
    uint8_t passed = (uint8_t)(m_rttFilterPass + m_ecnFilterPass + plFilterPass);

    if (passed >= 2)
    {
        m_sbMatchCounter++;
        if (m_sbMatchCounter >= 3)
            m_isSharedBottleneck = true;
    }
    else
    {
        if (m_sbMatchCounter > 0)
            m_sbMatchCounter--;
        if (m_sbMatchCounter == 0)
            m_isSharedBottleneck = false;
    }
	...
	struct sock *sk, const struct rate_sample *rs, u32 acked, u32 *new_cwnd)
{
	...
	if (rs->losses > 0) {
		...
		if (cwnd <= MPBBR_CWND_LOW_THRESHOLD) {
			mpbbr->num_dec = 0;
			new_cwnd = cwnd / 2;
		} else if (mpbbr->w_prev_max > cwnd) {
			mpbbr->num_dec++;
			s = max(50U, min(200U, mpbbr->w_prev_max / 32));
			
			if (mpbbr->num_dec >= MPBBR_N1 &&
				(mpbbr->w_prev_max - cwnd) >= s) {
				mpbbr->w_prev_max = 0;
				mpbbr->w_prev_min = 0;
				mpbbr->num_dec = 0;
				new_cwnd = cwnd / 2;
			} else {
				mpbbr->w_prev_min = cwnd;
				if (mpbbr->num_dec == MPBBR_N2) {
					mpbbr->w_prev_max = cwnd;
					mpbbr->num_dec = 0;
				}
				new_cwnd = (cwnd * lookup_beta(cwnd)) >> 10; // Assuming beta scaled in 1/1024 units
			}
		} else {
			mpbbr->w_prev_max = cwnd;
			mpbbr->w_prev_min = cwnd;
			mpbbr->num_dec = 0;
			new_cwnd = (cwnd * lookup_beta(cwnd)) >> 10;
		}
	
		cwnd = max(new_cwnd, 1U);
	}
 ...
 */
//changed the cwnd logic here to implement highspeed logic
	...
	u32 acked, u32 bw, int gain)
{
	...
	/* Allow recovery/restore to override cwnd directly */
	if (mpbbr_set_cwnd_to_recover_or_restore(sk, rs, acked, &cwnd))
		goto done;

	/* HighSpeed-style aggressive increase with RTT fairness */
	increase = TableLookupA(cwnd); // Unscaled alpha

	/* RTT-based fairness scaling */
	u32 curr_rtt = tp->srtt_us >> 3;     // current RTT in usec
	u32 min_rtt = mpbbr->min_rtt;        // MPBBR-tracked min RTT in usec

	if (cwnd > MPBBR_RTT_FAIRNESS_CWND_THRESH && min_rtt > 0) {
		double rtt_ratio = (double)curr_rtt / (double)min_rtt;
		double rtt_scaling = 1.0 / pow(rtt_ratio, MPBBR_RTT_FAIRNESS_FACTOR);
		increase = (u32)((double)increase * rtt_scaling);
	}

	cwnd += max((acked * increase) / cwnd, 1U);
	...
	/* If in PROBE_RTT, limit to minimum cwnd */
	if (mpbbr->mode == MPBBR_PROBE_RTT)
		tp->snd_cwnd = min(tp->snd_cwnd, mpbbr_cwnd_min_target);
}

//changes here
static void cmpbbr_spa_update_bad_paths(struct mptcp_cb *mpcb)
{
	double lp_max = 0.0;
	u32 rtt_min = UINT_MAX;

	// First pass: Find max LP and min RTT
	mptcp_for_each_sk(sk, mpcb) {
		struct tcp_sock *tp = tcp_sk(sk);
		struct mpbbr *ca = inet_csk_ca(sk);

		if (ca->min_rtt == 0)
			continue;

		double lp = (double)mpbbr_bw(sk) / pow((double)ca->min_rtt, 2);
		if (lp > lp_max)
			lp_max = lp;
		if (ca->min_rtt < rtt_min)
			rtt_min = ca->min_rtt;
	}

	if (lp_max == 0 || rtt_min == UINT_MAX)
		return;

	// Second pass: Classify subflows
	mptcp_for_each_sk(sk, mpcb) {
		struct tcp_sock *tp = tcp_sk(sk);
		struct mpbbr *ca = inet_csk_ca(sk);
		...
		double lp_i = (double)mpbbr_bw(sk) / pow((double)ca->min_rtt, 2);
		double Ri = (lp_max / pow((double)rtt_min, 2)) /
		            (lp_i / pow((double)ca->min_rtt, 2));

		if (Ri > MPBBR_SPA_THRESHOLD) {
			ca->is_bad_path = true;
			tp->snd_cwnd = MPBBR_BAD_PATH_CWND;
			tp->snd_ssthresh = MPBBR_BAD_PATH_CWND;
		} else {
			ca->is_bad_path = false;
		}
	}
}
